...
// Source: https://github.com/ceoloide/ergogen-footprints/blob/main/power_switch_smd_side.js
//
// Description:
...
module.exports = {
  ...
  body: (p) => {
    const common_start = `
      (module "ceoloide:power_switch_smd_side" (layer ${
        p.side
      }.Cu) (tedit 64473C6F)
    ...
    const silkscreen_front = `
      ...
      -90 + p.rot
        ...
        (fp_text user "ON" (at 0 ${p.invert_behavior ? "-" : ""}4.5 ${
      p.rot
    }) (layer "F.SilkS")
          (effects (font (size 1 1) (thickness 0.15)) (justify ${
            p.invert_behavior ? "bottom" : "top"
          }))
        )
        (fp_text user "OFF" (at 0 ${p.invert_behavior ? "" : "-"}4.5 ${
      p.rot
    }) (layer "F.SilkS")
          (effects (font (size 1 1) (thickness 0.15)) (justify ${
            p.invert_behavior ? "top" : "bottom"
          }))
        )
